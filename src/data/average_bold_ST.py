...
def make_sub_mask(dir_path, sub):
    ...
    nib.save(GM_mask, os.path.join(dir_path, 'output', 'masks', sub + '_GMmask_ST.nii.gz'))
    ...
    nib.save(sub_mask, os.path.join(dir_path, 'output', 'masks', sub + '_WholeBrain_ST.nii.gz'))
...
def flatten_epi(dir_path, sub, task_list, per_session=False):
    ...
    for task in task_list:
        ...
        for scan in scan_list:
            ...
            if per_session:
                savemat(os.path.join(dir_path, 'output', 'detrend', sub + '_epi_FULLbrain_' + task + '_sess' + str(sess_num) +'_ST.mat'), {sub + '_' + task : flat_bold_dt})
            else:
        ...
        if not per_session:
            ...
            savemat(os.path.join(dir_path, 'output', 'detrend', sub + '_epi_FULLbrain_' + task + '_ST.mat'), {sub + '_' + task : mean_bold})
...
if __name__ == '__main__':
    ...
    for sub in sub_list:
        ...
        if args.makemasks:
            ...
            sub_mask = nib.load(os.path.join(dir_path, 'output', 'masks', sub + '_WholeBrain_ST.nii.gz'))
