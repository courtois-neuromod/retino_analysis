...
def reassamble(data_path, code_path, sub, mask, num_vox, chunk_size, out):

    flat_output = np.zeros((num_vox,))
    file_path = sorted(glob.glob(os.path.join(data_path, 'results', sub, 'fullbrain_ST', 'sub' + sub[-2:] + '_fullbrain_ST_analyzePRF_' + out + '_*.mat')))

    for i in range(int(np.ceil(num_vox/chunk_size))):
        #print(out, i)
        flat_output[i*chunk_size:(i+1)*chunk_size] = loadmat(file_path[i])[out].reshape(-1,)

    savemat(os.path.join(code_path, 'results/analyzePRF', 'sub' + sub[-2:] + '_fullbrain_' + out + '.mat'), {'sub' + sub[-2:] + '_' + out: flat_output})
...
if __name__ == '__main__':
    ...
    # On elm/ginkgo
    data_path = '/home/mariestl/cneuromod/retinotopy/analyzePRF' if args.dir_path is None else args.dir_path
    code_path = '/home/mariestl/cneuromod/retinotopy/retino_analysis'
    ...
    mask = nib.load(os.path.join(code_path, 'output/masks', sub + '_WholeBrain_ST.nii.gz'))
    mask_dim = mask.get_fdata().shape

    '''
    As a reference...
    Number of voxels within "Whole Brain" mask outputed by average_bold_ST.py, per participant
    sub-01: 205276 voxels (w inclusive full brain mask)
    ...
    for out in out_list:
        reassamble(data_path, code_path, sub, mask, num_vox, chunk_size, out)

    # load, process and save volume of R^2 Values
    r2 = get_r2(code_path, sub, mask)
    ...
    get_rfsize(code_path, sub, conv_factor, mask, r2, args.threshold)
    get_angles_and_ecc(code_path, sub, conv_factor, mask, r2, args.threshold)
